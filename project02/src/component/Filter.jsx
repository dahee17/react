...
const Filter = () => {
    ...
    let newAlist = aList.map(item => <p key={item.name}>안녕하세요 저는 {item.name}이고, {item.bloodType}형입니다.</p>)

    // 실습
    // brithYear가 1994보다 작거나 같은 사람 만 출력 => 아이유 수지
    // 출력 방식 -> Map.jsx에 있는 코드 사용

    // find 함수
    // 배열의 요소를 순차적으로 순회하면서 조건에 일치하느 요소의 값을 즉시 반환
    ...
    let dataList = [ {
        ...
        let bList = dataList.filter(item => item.birthYear <= 1994)
        let newBlist = bList.map(item => <div key={item.imgSrc}>
