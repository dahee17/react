...
const Map = () => {
    ...
    //  Map 함수
    //  - 정진 함수를 호출한 결과를 통해 새로운 배열을 변환
    //  배열이름.map((매개변수) => (리턴식))
    // 반드시 key 값이 포함, 그 key값은 고유해야한다.
    // 그래서 key 값은 보통DB의 pk값, 고유번호 사용

    let num = [1, 2, 3, 4, 5]
    let newNum = num   // num 이라는 배열을 호출해서
                .map(  // 새로운 배열을 만듬
                    (item)=>  // 안에 있는 애들 싹 가져와서
                        (item*10) // 10 곱해서 내보갤거야 => newNum으로
                )
    console.log('newNum : ' , newNum)

    let tc = ['선영표', '장우영', '신정욱', '임승환']
    let newTc = tc.map(item => <button key={item}>{item}선생님</button>)
    console.log('nexTc', newTc)
    ...
    let List = list.map(item => 
        <p key = {item.name}>안녕하세요 저는 {item.name}입니다. 저는 {item.bloodType}이고 {item.age}살입니다.</p>
    )
    ...
    let newList = dataList.map(item => <div key={item.imgSrc}>
    ...
    <div>
        ...
        <hr />
         ...
         <div>
            {/* <img width='100px' src='https://mblogthumb-phinf.pstatic.net/MjAxNzA5MDdfMjQ1/MDAxNTA0NzY2MTg3NjMw.082G5PddLUVM6x35yJ5Ex6lKonxHrf_tf7lJHjEijYsg.WSzLj8tTAdxrZqXxMMutfLdOE-P7oNKK80_asLRlEmAg.PNG.morning1377/%EC%95%84%EC%9D%B4%EC%9C%A0_%EC%9D%B8%EC%8A%A4%ED%83%80%EA%B7%B8%EB%9E%A8_%EA%B7%BC%ED%99%A9.PNG?type=w2'></img>
            <h2>아이유</h2> 
            <p>
                대한민국 가수 겸 배우.. 
            </p>
